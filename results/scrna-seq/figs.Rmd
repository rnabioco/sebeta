...
save_plot("ENTPD3_ITGA1_comparison_sorted.pdf", plt, base_aspect_ratio = 1.2, nrow = 1, ncol = 3)
...
save_plot("ENTPD3_ITGA1_comparison_reagg.pdf", plt, base_aspect_ratio = 1.2, nrow = 1, ncol = 3)
...
bcells$Reaggregated <- CellCycleScoring(
  object = bcells$Reaggregated,
  g2m.features = cc.genes$g2m.genes,
  s.features = cc.genes$s.genes
)

plot_feature(bcells$Reaggregated, "Phase")
bcells$Reaggregated$CC.Difference <- bcells$Reaggregated$S.Score - bcells$Reaggregated$G2M.Score
...
run_monocle <- function(obj, markers, id, 
  ...
  atm <- reduceDimension(atm, reduction_method = 'DDRTree', 
                         residualModelFormulaStr = as.formula( '~S.Score + G2M.Score'))
...
```{r}
bcells <- map(c("Sorted", "Reaggregated"), ~readRDS(paste0(.x, "_final.rds")))
